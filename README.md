...
`build-redis.yml` and `build-memcached.yml` do the same with one recipe and six; `build-ruby.yml`
runs six legs across two recipes; and `publish-index.yml` regenerates and signs the index from every
release that exists.

The last two of those are the ones where two of the six legs produce nothing by design, and that is
the whole reason they are still in the matrix — see the Redis and Memcached section above.
